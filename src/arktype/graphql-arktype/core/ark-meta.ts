...
/**
 * Metadata key under which the originating ArkType schema is attached to a
 * generated class (input, object, args). Pipes look it up to validate input,
 * resolvers can opt into output validation by reading the same key.
 */
...
export function getArkKind(target: Function): ArkClassKind | undefined {
  return Reflect.getMetadata(ARK_KIND_METADATA, target);
}

/**
 * Module-scoped registry mapping ArkType schemas (by reference identity) to
 * the GraphQL classes generated from them. Lets nested-type resolution work
 * without callers having to thread refs through every factory call.
 *
 * Identity-based: the same ArkType `type({...})` value always produces the
 * same key. Schemas built ad-hoc in a property override won't be found here
 * — that's the user's problem to wire via the explicit `fields:` map.
 */
class ArkSchemaRegistry {
  ...
  register(schema: ArkType<any>, cls: any, name: string): void {
    ...
    try {
      ...
      if (!this.byJsonSig.has(sig)) this.byJsonSig.set(sig, cls);
    } catch {
