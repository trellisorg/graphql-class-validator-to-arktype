...
export interface CreateArkInputTypeOptions {
  ...
  description?: string;
  /**
   * Override GraphQL field types for properties that can't be inferred from
   * JSON schema alone (object refs, array-of-object refs) or where you want
   * to force a specific type (e.g. ID, custom scalar).
   *
...
/**
 * Generate a NestJS `@InputType` class from an ArkType schema. The schema is
 * also attached to the class so `ArkValidationPipe` can run it on the request.
 */
